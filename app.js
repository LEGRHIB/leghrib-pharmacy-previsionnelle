...
function renderImport(el){
  ...
  el.innerHTML=`
    ...
    <div class="import-status-grid">
      ...
      <div class="import-status"><span class="dot ${s.rotation?'dot-green':'dot-gray'}"></span>${s.rotation?`✓ Rotation annuelle — ${DB.rotation.length} produits`:'◯ Rotation annuelle (DCI, labo)'}</div>
    ...
    <div style="margin-top:24px;text-align:center">
      <button class="btn btn-primary" onclick="runCompute()" style="padding:12px 32px;font-size:15px" ${!s.nomenclature&&!s.rotation?'disabled style="opacity:.5;padding:12px 32px;font-size:15px"':''}>🔄 Calculer les Prévisions</button>
...
function renderAlerts(el){
  ...
  el.innerHTML=`
    ...
    <p class="page-subtitle">${fmt(ps.length)} produits — triés par score de risque</p>
    <div class="table-wrap">
      <div class="table-toolbar">
        <input placeholder="🔍 Rechercher produit, DCI, labo..." value="${alertsFilter.search}" oninput="alertsFilter.search=this.value;alertsPage=0;renderAlerts(document.getElementById('mainContent'))">
      ...
      ${tp>1?`<div class="pagination"><button ${alertsPage===0?'disabled':''} onclick="alertsPage=0;renderAlerts(document.getElementById('mainContent'))">«</button><button ${alertsPage===0?'disabled':''} onclick="alertsPage--;renderAlerts(document.getElementById('mainContent'))">‹</button><span class="page-info">Page ${alertsPage+1}/${tp} (${fmt(ps.length)})</span><button ${alertsPage>=tp-1?'disabled':''} onclick="alertsPage++;renderAlerts(document.getElementById('mainContent'))">›</button><button ${alertsPage>=tp-1?'disabled':''} onclick="alertsPage=${tp-1};renderAlerts(document.getElementById('mainContent'))">»</button></div>`:''}
    </div>`;
}
...
function renderSettings(el){
  ...
  el.innerHTML=`
    ...
    <div class="settings-grid">
      ...
      <div class="setting-group" style="grid-column:span 2"><h3>🔄 Actions</h3>
